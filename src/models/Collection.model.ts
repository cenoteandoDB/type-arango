 ...
 */
export function findCollectionForContainer(
  someClass: any
): Collection | undefined {
  return collections.find(
    (c) => someClass === c.Class || someClass.prototype instanceof c.Class
  );
}
...
export class Collection {
  ...
  static toName(input: string) {
    return config.prefixCollectionName
      ? module.context.collectionName(input)
      : input;
  }
  ...
  get routeAuths() {
    return (this.decorator["Route.auth"] || [])
      .map((d) => d.authorizeFunction)
      .reverse();
  }

  get routeRoles() {
    return (this.decorator["Route.roles"] || [])
      .map((d) => d.rolesFunction)
      .reverse();
  }
  ...
  finalize() {
    ...
    const doc = (this.doc = getDocumentForContainer(
      argumentResolve(ofDocument)
    ));
    doc.col = this;
    ...
    if (isActive) {
      ...
      // setup AQLFunctions
      if (Function)
        for (let { prototype, attribute, name, isDeterministic } of Function) {
    ...
    if (Route)
      for (let {
        ...
        let schema: any;
        const a: any = argumentResolve(
          pathOrRolesOrFunctionOrOptions,
          (inp: any) => enjoi(inp, "required"),
          Joi
        );
        ...
        if (schema) {
          ...
          if (schema._type === "object") {
            ...
            schema =
              schema._flags.presence === "optional"
                ? schema
                : schema.required();
            ...
            for (const attr of schema._inner.children) {
              ...
              if (opt.pathParams.find((p) => p[0] === attr.key)) {
                ...
                opt.pathParams = opt.pathParams.map((p) =>
                  p[0] === attr.key
                    ? ([p[0], attr.schema, p[2]] as RoutePathParam)
                    : p
                );
                // remove attr from schema
                schema._inner.children = schema._inner.children.filter(
                ...
                opt.queryParams = opt.queryParams.map((p) =>
                  p[0] === attr.key
                    ? ([p[0], attr.schema, p[2]] as RoutePathParam)
                    : p
                );
              ...
              if (METHODS_BODYLESS.includes(method)) {
                ...
                opt.queryParams.push([
                  attr.key,
                  attr.schema,
                  Scalar.iconRequired(isRequired) +
                    " " +
                    (attr.schema._description
                      ? "**" + attr.schema._description + "**"
                      : (isRequired ? "**Required" : "**Optional") +
                        ` query parameter** ${
                          operators ? "with operator" : ""
                        }  \`[ ${attr.key}: ${
                          operators ? "[operator, value]" : attr.schema._type
                        } ]\``) +
                    (operators
                      ? `
　 \`Operators: ${operators.join(", ")}\``
				...
				　 \`Example: ?${attr.key}=${
                      operators
                        ? arraySample(operators) + config.paramOperatorSeparator
                        : ""
                    }${attr.schema._examples[0] || attr.schema._type}\``,
                ]);
              }
            }
