...
function mapToInstances<T>(
  Class: new (doc: DocumentData, docIsSync: boolean) => T,
  results: any[]
) {
...
export class Entities {
  ...
  static findOne(
    keyOrOptions: QueryOpt | string | number,
    options: QueryOpt = {}
  ): any {
    if (typeof keyOrOptions === "string" || typeof keyOrOptions === "number") {
      if (options.filter)
        options.filter = Object.assign(options.filter, { _key: keyOrOptions });
      else options = Object.assign(options, { filter: { _key: keyOrOptions } });
    } else if (keyOrOptions) options = keyOrOptions;
