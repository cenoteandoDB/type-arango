 ...
 */
function createDocumentFromContainer<T = typeof Entity>(
  someClass: new () => T
): Document<T> {
 ...
 */
export function findDocumentForContainer<T = typeof Entity>(
  someClass: new () => T
 ...
 */
export function getDocumentForContainer<T = typeof Entity>(
  someClass: new () => T
): Document<T> {
...
export class Document<T = any> {
  ...
  public emitBefore(method: EventMethod, ...args: any[]) {
    // PropertyDecorator
    toArray(
      this.decorator[("Before." + method + ".prop") as DecoratorId]
    ).forEach(({ attribute, resolver }) => {
      switch (method) {
        case "document":
          resolver(args[0], { _key: args[0], attribute, method });
          break;
        case "insert":
          args[0][attribute] = resolver(args[0][attribute], {
            json: args[0],
            attribute,
            method,
          });
    ...
    // ClassDecorator
    toArray(
      this.decorator[("Before." + method + ".class") as DecoratorId]
    ).forEach(({ resolver }) => {
      let v: false | true | undefined | ArangoDB.HttpStatus;
      switch (method) {
        case "document":
          v = resolver(args[0], { _key: args[0], method });
          break;
        case "insert":
          v = resolver(args[0], { json: args[0], method });
          break;
        case "update":
          v = resolver(args[0], { _key: args[1], json: args[0], method });
          break;
        case "replace":
          v = resolver(args[0], { _key: args[1], json: args[0], method });
          break;
  ...
  public emitAfter(method: EventMethod, ...args: any[]) {
    // PropertyDecorator
    toArray(
      this.decorator[("After." + method + ".prop") as DecoratorId]
    ).forEach(({ attribute, resolver }) => {
      switch (method) {
        case "document":
          args[0][attribute] = resolver(args[0][attribute], {
    ...
    // ClassDecorator
    toArray(
      this.decorator[("After." + method + ".class") as DecoratorId]
    ).forEach(({ resolver }) => {
      let v: false | true | undefined | ArangoDB.HttpStatus;
      switch (method) {
        case "document":
        ...
        case "insert":
          ...
          break;
        case "update":
          v = resolver(args[1], { _key: args[1], document: args[0], method });
          break;
        case "replace":
          v = resolver(args[1], { _key: args[1], document: args[0], method });
          break;
  ...
  resolveRelation(
    ...
    if (rel.document.isEdge && edgeAttributes.includes(rel.attribute)) {
      ...
      if (data._key) filter[rel.attribute] = ["HAS", data._key];
    }
    ...
    if (ref) {
      if (isObject(ref))
        throw new Error(
          `Invalid relation value of "${rel.document.name}.${
            rel.attribute
          }": ${JSON.stringify(ref)}`
        );
      ...
      if (this.isEdge) {
        ...
        else ref = ref.replace(rel.document.col!.name + "/", "");
      }
      filter = { _key: ref };
    }
    ...
    if (!Object.keys(filter).length)
      throw new Error(
        `Cannot resolve relation of "${rel.document.name}.${rel.attribute}": empty filter`
      );

    const entities = rel.document.col!.Class as typeof Entities;

    if (arg === null) return data[attribute];
    ...
    switch (rel.type) {
      default:
        return null;
      case "OneToOne":
        return entities.findOne(opt);
      case "OneToMany":
        return entities.find(opt);
    }
  }
    ...
    for (let role of this.roles) {
      ...
      for (let attr of attributes) {
        ...
        if (readers && !readers.includes(role))
          roles.read = concatUnique(roles.read, attr.attribute);
        if (writers && !writers.includes(role))
          roles.write = concatUnique(roles.write, attr.attribute);
      }
  ...
  stripAttributeList(providedRoles: Roles, method: "read" | "write") {
    ...
    Object.keys(attributes).forEach(
      (k) => attributes[k] === sum && stripAttributes.push(k)
    );
  ...
  complete() {
    ...
    if (Attribute)
      for (let { attribute } of [...(Authorized || []), ...Attribute]) {
  ...
  finalize() {
    ...
    if (OneToOne || OneToMany)
      for (let { decorator, prototype, attribute, type, relation } of (
        OneToOne || []
      ).concat(OneToMany || [])) {
        metadata =
          attribute && Reflect.getMetadata("design:type", prototype, attribute);

        if (
          !type &&
          ((metadata && ["Array", "Function"].includes(metadata.name)) ||
            (!type && !metadata))
        )
        ...
        this.relation[attribute!] = {
          type: decorator as RelationType,
    ...
    // @Authorized
    if (Authorized)
      for (let o of Authorized) {
        const A = this.decorator.Attribute || [];
        let a = (A || []).find((a) => a.attribute === o.attribute);
        if (!a)
          a = this.decorate("Attribute", o).find(
            (attr) => attr.attribute === o.attribute
          );
        else {
    ...
    if (Attribute)
      for (let {
        prototype,
        attribute,
        typeOrRequiredOrSchemaOrReadersOrFunction,
        readersArrayOrFunction,
        writersArrayOrFunction,
      } of Attribute) {
        ...
        if (!rel) joi = toJoi(metadata);
          ...
          switch (rel.type) {
            case "OneToOne":
              joi =
                rel.document.attribute[rel.attribute].schema || Joi.string();
              break;
            case "OneToMany":
              joi = Joi.array().items(
                rel.document.attribute[rel.attribute!].schema || Joi.string()
        ...
        if (readers)
          roles = removeValues(
            {
              readers: readers,
              writers: writers || config.requiredWriterRolesFallback,
            },
            undefined
          );
        ...
        const data: AttributeObject = removeValues(
          { attribute, roles, schema, metadata },
          undefined
        );
        ...
        if (metadata && metadata._typeArango) {
          if (version < metadata._typeArango) {
            logger.error(
              `Type.${metadata.name} requires type-arango v${metadata._typeArango}`
            );
          } else {
            if (metadata.forClient)
              this.forClientMap.push([attribute!, metadata.forClient]);
            if (metadata.fromClient)
              this.fromClientMap.push([attribute!, metadata.fromClient]);
            eventTypes.forEach((name) => {
              const m = metadata[eventNames[eventTypes.indexOf(name)]];
              if (!m) return;
              this.decorate((name + ".prop") as DecoratorId, {
                prototype,
                attribute,
                resolver: m,
              });
