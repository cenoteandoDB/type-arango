...
export interface CreateCollectionOptions
  extends ArangoDB.CreateCollectionOptions,
    Partial<RouteGroups> {
  name?: CollectionName;
  of?: typeof Entity;
...
export type CollectionRouteArray = [
  RouteDecorator,
  string | CollectionRoute | SchemaFunc,
  ...any[]
];
...
export type SchemaFunc = (
  enjoi: (type?: any) => typeof Joi | any,
  joi?: any
) => typeof Joi | boolean | Object;
...
export type ValidateSchemaFunc = (
  returns: AnyJoiSchema,
  enjoi: Enjoi
) => AnyJoiSchema;
...
export interface RouteRolesArg {
  ...
  ) => ArangoDB.InsertResult;
  method: RouteMethod;
  name: string;
  param: { [key: string]: any };
  path: string;
  query: (
    query: ArangoDB.Query,
  ...
  update: (
    selector: string | ArangoDB.DocumentLike | DocumentData,
    data?: DocumentData,
...
export interface RouteArg extends RouteRolesArg, RouteBaseOpt {
  ...
  send: (
    ...
    omitUnreadableAttributes?: boolean
  ) => Foxx.Response; // send role specific response
}
...
type QueryFilterValue =
  ...
  | number
  | boolean
  | [QueryFilterOperator, string | number | boolean]
